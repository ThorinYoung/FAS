# this file is located at /fomm_models/video-preprcessing/crop_nemo.py, use that file instead
import pynvml
import cv2
import numpy as np
import pandas as pd
...
from fomm_models.video_preprocessing.util import bb_intersection_over_union, join, scheduler, crop_bbox_from_frames, save
...
def store(frame_list, tube_bbox, video_id, args):
    ...
    save(os.path.join(args.out_folder, partition, name), out, args.format, args.fps)
    return [{'bbox': '-'.join(map(str, final_bbox)), 'start': 0, 'end': len(frame_list), 'fps': args.fps,
...
def process_video(video_id, args):
    ...
    _vid = cv2.VideoCapture(video_path)
    fps = int(round(_vid.get(cv2.CAP_PROP_FPS)))
    args.fps = fps
...
if __name__ == "__main__":
    ...
    parser.add_argument("--in_folder", default = 'test')
    parser.add_argument("--out_folder", default = 'res')
